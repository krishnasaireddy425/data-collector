"""
PaperStrategyV9 — V4's EV regression strategy with ONE change:
stricter EV threshold (0.04 vs V4's 0.025).

No V3 filter. No exit logic. Just V4 + higher confidence bar.

Purpose: isolate the effect of V4's own EV threshold, independent of
the V3 probability filter (which is tested separately in V4+).

Rationale:
  V4 today fires at EV > $0.025/share — barely above noise given V4's
  val MAE of ~$0.35. Raising the bar to $0.04 keeps V4's cheap-side
  character but only acts on more confident mispricing signals.
...
Supabase tables (schema in sql/009_create_v9_clean.sql):
  v9_windows, v9_events
"""
...
MIN_ENTRY = 30
MAX_ENTRY = 240
EV_THRESHOLD = 0.04           # NEW: stricter than V4's 0.025
PREDICT_EVERY_SEC = 1.0

# Column whitelist for v9_events
V9_EVENTS_COLS = {
...
class PaperStrategyV9:
    def __init__(self, slug, open_epoch, close_epoch,
                 predictor: EVPredictor, logger):
        self.slug = slug
    ...
    def _enter(self, side, price, elapsed, predicted_ev, ev_up, ev_down, book):
        ...
        print(f"  [V9] {self.slug} ENTRY {side} x{SHARES} @ ${price:.3f}  "
    ...
    def settle(self, btc_open=None, btc_close=None, winner=None):
        ...
        if self.entry_side is None:
            print(f"  [V9] {self.slug} SKIPPED  winner={winner_norm}")
        ...
        print(f"  [V9] {self.slug} SETTLE  side={self.entry_side}  winner={winner_norm}  "
