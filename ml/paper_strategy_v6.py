"""
PaperStrategyV6 — same 44-feature continuous XGB model as V3, but with
CONF_THRESHOLD lowered from 0.90 → 0.70. Enters earlier at cheaper prices
for more coverage.

Backtest on 1,260 CSVs (epoch ≥ 1776156900):
  entered 1,239 (98.3%)  accuracy 76.1%  PnL +$361.30  avg entry $0.73 @ t=97s

Shares the same xgb_sec.joblib as V3 (independent PredictorSec instance so
internal tick buffer is not shared).
...
Entry rule:
  ...
  Once entered: HOLD to settlement. No exits, no hedging.

Supabase tables (schema in sql/005_create_v6_clean.sql):
  v6_windows, v6_events
"""
...
MIN_ENTRY = 60
MAX_ENTRY = 240
CONF_THRESHOLD = 0.70      # <-- only difference from V3
PREDICT_EVERY_SEC = 1.0

# Column whitelist for v6_events — matches sql/005_create_v6_clean.sql
V6_EVENTS_COLS = {
...
class PaperStrategyV6:
    ...
    def _enter(self, side, price, elapsed, confidence, book):
        ...
        print(f"  [V6] {self.slug} ENTRY {side} x{SHARES} @ ${price:.3f}  "
    ...
    def settle(self, btc_open=None, btc_close=None, winner=None):
        ...
        if self.entry_side is None:
            print(f"  [V6] {self.slug} SKIPPED  winner={winner_norm}")
        ...
        print(f"  [V6] {self.slug} SETTLE  side={self.entry_side}  winner={winner_norm}  "
